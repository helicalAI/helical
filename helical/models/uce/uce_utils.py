...
def get_positions(
    species_chrom_csv_path: Path, species: str, adata: sc.AnnData
    ...
    genes_to_chroms_pos["spec_chrom"] = pd.Categorical(
        genes_to_chroms_pos["species"] + "_" + genes_to_chroms_pos["chromosome"]
    )  # add the spec_chrom list
    spec_gene_chrom_pos = genes_to_chroms_pos[
        genes_to_chroms_pos["species"] == species
    ].set_index("gene_symbol")

    filtered_spec_gene_chrom_pos = spec_gene_chrom_pos.loc[
        [k.upper() for k in adata.var_names]
    ]
...
def get_protein_embeddings_idxs(
    ...
    return torch.tensor(
        [spec_all_genes.index(k.lower()) + offset for k in adata.var_names]
    ).long()


def prepare_expression_counts_file(
    gene_expression: np.array, name: str, folder_path: str = "./"
) -> None:
    """
...
def load_model(
    model_path: Union[str, Path], model_config: Dict[str, str], all_pe: torch.Tensor
) -> TransformerModel:
    """
