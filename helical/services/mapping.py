...
def map_gene_symbols_to_ensembl_ids(adata: AnnData, gene_names: str):
    """
    ...
    ensembl_release = EnsemblRelease(species=human) 
    ...
    for index, name in adata.var[gene_names].items():
        try:
            adata.var.at[index, 'ensembl_id'] = grch38.gene_ids_of_gene_name(name)[0]
        except:
            continue
...
def map_ensembl_ids_to_gene_symbols(adata: AnnData, ensembl_id_key: str = "ensembl_id"):
    """
    ...
    for index, ensembl_id in adata.var[ensembl_id_key].items():
        try:
            adata.var.at[index, 'gene_names'] = grch38.gene_name_of_gene_id(ensembl_id)
        except:
            continue
